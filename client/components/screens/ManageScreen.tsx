...
import { api, type Farm as ApiFarm, type Pond as ApiPond, type ScanResult } from "@/utils/api";
...
export function ManageScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const apiFarms: ApiFarm[] = farmsResponse.farms || [];

      // Fetch all ponds for all farms
      const allPonds: ApiPond[] = [];
      ...
      // Fetch all scans
      const scansResponse = await api.getScans();
      const apiScans: ScanResult[] = scansResponse.scans || [];

      // Convert API data to local format
      const convertedFarms: Farm[] = apiFarms.map(f => ({
        id: f._id,
        name: f.name,
        location: f.location,
        pondCount: allPonds.filter(p => p.farm === f._id).length,
        totalArea: allPonds.filter(p => p.farm === f._id).reduce((sum, p) => sum + (p.area || 0), 0),
        createdDate: f.createdAt,
      }));

      const convertedPonds: Pond[] = allPonds.map(p => {
        const pondScans = apiScans.filter(s => s.pond === p._id);
        return {
          id: p._id,
          farmId: p.farm,
          name: p.name,
          size: p.area || 0,
          shrimpCount: 0, // Not tracked in API
          lastScanDate: pondScans.length > 0 ? pondScans[0].createdAt : undefined,
        };
      });

      const convertedScans: ScanRecord[] = apiScans.map(s => ({
        id: s._id,
        pondId: s.pond || '',
        date: s.createdAt,
        healthStatus: s.healthScore
          ? (s.healthScore >= 80 ? 'excellent' : s.healthScore >= 60 ? 'good' : s.healthScore >= 40 ? 'fair' : 'poor')
          : 'good',
        metrics: s.metrics,
      }));

      setFarms(convertedFarms);
      setPonds(convertedPonds);
      setScanHistory(convertedScans);
    } catch (error: any) {
  ...
  const handleAddFarm = async () => {
    if (newFarmName.trim()) {
      try {
        setIsLoading(true);
        await api.createFarm(newFarmName, newFarmLocation);
        setNewFarmName('');
        setNewFarmLocation('');
        setShowAddFarmModal(false);
        await loadData();
        Alert.alert('Thành công', 'Đã tạo nông trại mới');
      } catch (error: any) {
        console.error('Failed to create farm:', error);
        Alert.alert('Lỗi', 'Không thể tạo nông trại');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleAddPond = async () => {
    if (newPondName.trim() && selectedFarmId) {
      try {
        setIsLoading(true);
        await api.createPond(selectedFarmId, newPondName, 0);
        setNewPondName('');
        setSelectedFarmId('');
        setShowAddPondModal(false);
        await loadData();
        Alert.alert('Thành công', 'Đã tạo ao mới');
      } catch (error: any) {
        console.error('Failed to create pond:', error);
        Alert.alert('Lỗi', 'Không thể tạo ao');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      {/* Loading Overlay */}
      {isLoading && !showAddPondModal && !showAddFarmModal && (
        <View
          style={{
            position: 'absolute',
            top: 0,
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  Alert.alert('Thông báo', 'Thêm ao thành công')
                  setShowAddPondModal(false)
                } }
              >
